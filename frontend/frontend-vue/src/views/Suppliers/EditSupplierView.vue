<template>
  <Layout title="Edit Supplier">
    <div class="sup-form-page">
      ...
      <!-- LOADING SKELETON -->
      <div v-if="loading" class="form-card form-card--skeleton">
        ...
        <div class="skeleton-body">
          ...
          <div class="skeleton-field skeleton-field--tall"></div>
        </div>
      </div>

      <!-- FORM -->
      <form v-else @submit.prevent="updateSupplier" class="sup-form">
        <div class="form-card">
          <div class="form-card-header">
            ...
            <div>
              ...
              <p class="form-card-sub">Update the details for this supplier.</p>
            </div>
          </div>
...
const form = ref({ SupplierName: '', ContactNumber: '', Email: '', Address: '' })
...
async function updateSupplier() {
  ...
  try {
    await api.post(`/api/suppliers/${supplierId}`, { ...form.value, _method: 'PUT' })
...
<style scoped>
/* Same token bridge + form styles as Create — identical design */
.sup-form-page {
  --s-bg:             var(--c-bg);
...
.breadcrumb-link {
  ...
  color: var(--s-text-muted); text-decoration: none; transition: color .15s;
}
...
/* SKELETON */
.form-card--skeleton { padding: 24px; }
.skeleton-header { display: flex; align-items: center; gap: 14px; margin-bottom: 24px; }
.skeleton-icon  {
  width: 40px; height: 40px; border-radius: 11px; flex-shrink: 0;
  background: var(--s-border); animation: shimmer 1.4s infinite;
}
.skeleton-lines { display: flex; flex-direction: column; gap: 8px; }
.skeleton-line  { height: 12px; border-radius: 6px; background: var(--s-border); animation: shimmer 1.4s infinite; }
.skeleton-line--wide   { width: 160px; }
.skeleton-line--narrow { width: 100px; }
.skeleton-body  { display: flex; flex-direction: column; gap: 16px; }
.skeleton-field { height: 42px; border-radius: 10px; background: var(--s-border); animation: shimmer 1.4s infinite; }
.skeleton-field--tall { height: 90px; }
...
.input-icon          { position: absolute; left: 13px; font-size: 12px; color: var(--s-text-faint); pointer-events: none; z-index: 1; transition: color .22s; }
...
.form-input:focus { border-color: var(--s-accent); box-shadow: 0 0 0 3px var(--s-ring); background: var(--s-surface); }
.form-textarea { height: auto; padding-top: 11px; padding-bottom: 11px; resize: vertical; line-height: 1.6; }
